...
function AccessoriesDetailsPage() {
  ...
  await fetch(`https://production-project-1.onrender.com/api/accessories/${id}/reviews`, {
        method: "POST",
  ...
  const res = await fetch(`https://production-project-1.onrender.com/api/accessories/${id}`);
  const data = await res.json();
      setAccessory(data);
      setRatingCount(data.ratingCount || 0);
      setAvgRating(data.avgRating || null);
